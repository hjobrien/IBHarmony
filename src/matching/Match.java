...
public class Match {
	...
	public Match (Person p1, Person p2, ArrayList<Integer> weights){
		this.p1 = p1;
		this.p2 = p2;
		this.matchScore = getGoodFitCount(p1, p2, weights);
	}
	
	public static int getGoodFitCount(Person p1, Person p2, ArrayList<Integer> weights) {
		...
		for(int k = 0; k < p1Answers.length(); k++){
			if(p1Answers.charAt(k) == p2Answers.charAt(k)){
				...
				tempGoodFitCount+=weights.get(k);
			}
		}
